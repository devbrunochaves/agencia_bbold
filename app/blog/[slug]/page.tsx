    ...
    <main className="pt-28 pb-24 px-10">
      <div className="max-w-[720px] mx-auto">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-6">
          ...
          {post.publishedAt && (
            <span className="text-sm text-offwhite/35">{formatDate(post.publishedAt)}</span>
          )}
          {post.readTime && (
            <span className="text-sm text-offwhite/35">{post.readTime} min de leitura</span>
          )}
        </div>

        {/* Title */}
        <h1 className="font-display text-[clamp(2.2rem,6vw,3.8rem)] leading-[1.05] tracking-wide text-offwhite mb-6">
          {post.title}
        </h1>

        {/* Excerpt */}
        {post.excerpt && (
          <p className="text-lg text-offwhite/55 leading-relaxed mb-8 border-l-4 border-yellow pl-5">
            {post.excerpt}
          </p>
        )}
        ...
        <div className="mt-16 p-8 bg-yellow rounded-lg text-center">
          ...
          <p className="text-black/65 mb-6">
            Vamos conversar sobre como aplicar isso no seu negócio.
          </p>
          <Link
            href="/#contato"
